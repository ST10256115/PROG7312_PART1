...
namespace MunicipalServicesApp.Data
{
    public static class EventStore
    {
        ...
        private static readonly HashTable<string, DynamicArray<EventItem>> _byCategory = new HashTable<string, DynamicArray<EventItem>>();
        private static readonly CustomSet<string> _categories = new CustomSet<string>();
        private static readonly PriorityQueue<EventItem> _upcoming = new PriorityQueue<EventItem>(
        ...
        public static EventItem[] Search(string query, string category, DateTime? from, DateTime? to)
        {
            ...
            foreach (var ev in pool)
            {
                ...
                if (from.HasValue && ev.StartUtc < from.Value.ToUniversalTime()) continue;
                if (to.HasValue && ev.StartUtc > to.Value.ToUniversalTime()) continue;
        ...
        private static void LoadOrSeed()
        {
            ...
            Ingest(seed);
            try { File.WriteAllText(DbPath, JsonConvert.SerializeObject(seed, Formatting.Indented)); } catch { }
        }
